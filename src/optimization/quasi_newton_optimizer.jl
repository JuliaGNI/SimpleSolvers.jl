...
function QuasiNewtonOptimizer(x::VT, F::Callable; ∇F!::Union{Callable,Nothing} = nothing, hessian = HessianParametersAD, linesearch = Bisection, config = Options()) where {XT, VT <: AbstractVector{XT}}
