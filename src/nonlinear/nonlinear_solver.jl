...
It's arguments are:
- `nls::`[`NonlinearProblem`](@ref): the system that has to be solved. This can be accessed by calling [`nonlinearproblem`](@ref),
...
struct NonlinearSolver{T,MT<:NonlinearSolverMethod,AT,NLST<:NonlinearProblem{T},LST<:AbstractLinearProblem,JT<:Jacobian{T},LSoT<:AbstractLinearSolver,LiSeT<:LinesearchState{T},CT<:NonlinearSolverCache{T},NSST<:NonlinearSolverStatus{T}} <: AbstractSolver
    ...
    function NonlinearSolver(x::AT, nls::NLST, ls::LST, linearsolver::LSoT, linesearch::LiSeT, cache::CT; method::MT = NewtonMethod(), jacobian::JT=JacobianAutodiff(nls.F, x), options_kwargs...) where {T,AT<:AbstractVector{T},MT<:NonlinearSolverMethod,JT<:Jacobian,NLST,LST,LSoT,LiSeT,CT}
        status = NonlinearSolverStatus(x)
        config = Options(T; options_kwargs...)
        new{T,MT,AT,NLST,LST,JT,LSoT,LiSeT,CT,typeof(status)}(nls, ls, jacobian, linearsolver, linesearch, method, cache, config, status)
    end
end
...
jacobian!(s::NonlinearSolver{T}, x::AbstractVector{T}, params) where {T} = jacobian!(nonlinearproblem(s), Jacobian(s), x, params)
