"""
    struct LapackSVDSolver{RT} <: RankRevealingMethod{RT}
...
Like [`LapackPivotedQR`](@ref) it returns the **minimum-norm solution** rather than raising a
...
LapackSVDSolver{Missing}(missing)
```
...
```jldoctest; setup = :(using SimpleSolvers)
LapackSVDSolver(; rtol = 1e-10)

# output

LapackSVDSolver{Float64}(1.0e-10)
```
...
The decomposition is the expensive one — 2–4× [`LapackPivotedQR`](@ref)'s, and it allocates heavily,
...
[`LapackPivotedQR`](@ref)'s applies two blocked orthogonal factors to a single column and allocates
its own LAPACK workspace each time.

So the sum favours [`LapackPivotedQR`](@ref) at one solve per factorization — a Newton step — and
...
is in [`LapackPivotedQR`](@ref)'s docstring.
...
`ComplexF64`), and to square matrices, exactly as [`LapackPivotedQR`](@ref) is.
[`SVDSolver`](@ref) is the pure-Julia counterpart that takes any floating-point type.
...
julia> ls = LinearSolver(LapackSVDSolver(), A);
...
struct LapackSVDSolver{RT} <: RankRevealingMethod{RT}
    ...
    LapackSVDSolver(; rtol = missing) = new{typeof(rtol)}(rtol)
end

"""
    LapackSVDCache <: LinearSolverCache

The cache of an [`LapackSVDSolver`](@ref).
...
factorized.
"""
mutable struct LapackSVDCache{T, RT <: Real, AT <: AbstractMatrix{T}} <:
               LinearSolverCache{T}
    A::AT
    U::Matrix{T}
    S::Vector{RT}
...
function LinearSolverCache(method::LapackSVDSolver, A::AbstractMatrix{T}) where {T}
    _blas_eltype_check(method, T)
    n = checksquare(A)
    Ā = Matrix{T}(A)
    LapackSVDCache{T, real(T), typeof(Ā)}(
        Ā, zeros(T, n, n), zeros(real(T), n), zeros(T, n, n),
        zeros(T, n), 0, false)
end

function _decompose!(method::LapackSVDSolver, c::LapackSVDCache{T}) where {T}
...
function singular_values(lsolver::LinearSolver{T, LSM}) where {T, LSM <: LapackSVDSolver}
    checkfactorized(lsolver)
    cache(lsolver).S
end

function LinearAlgebra.ldiv!(x::AbstractVector{T}, lsolver::LinearSolver{T, LSM},
        b::AbstractVector{T}) where {T, LSM <: LapackSVDSolver}
    c = cache(lsolver)
    checkfactorized(lsolver)
