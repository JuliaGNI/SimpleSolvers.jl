"""
    struct LapackPivotedQR{RT} <: RankRevealingMethod{RT}
...
LapackPivotedQR{Missing}(missing)
```
...
```jldoctest; setup = :(using SimpleSolvers)
LapackPivotedQR(; rtol = 1e-10)

# output

LapackPivotedQR{Float64}(1.0e-10)
```
...
[`LapackSVDSolver`](@ref) computes the same minimum-norm solution and additionally reports the
...
[`LapackSVDSolver`](@ref) for it. This method is the cheaper of the two *per factorization*, and the
...
| | `LapackPivotedQR` | **1.83** | 6000 | 0.71 | 98496 |
| | [`LapackSVDSolver`](@ref) | 8.12 | 11392 | **0.08** | **0** |
| 40 | [`LapackLU`](@ref)\\* | 5.46 | 0 | 0.58 | 0 |
| | `LapackPivotedQR` | **19.6** | 17824 | 8.83 | 98496 |
| | [`LapackSVDSolver`](@ref) | 72.5 | 81072 | **0.38** | **0** |
| 128 | [`LapackLU`](@ref)\\* | 64.5 | 0 | 3.12 | 0 |
| | `LapackPivotedQR` | **352** | 66304 | 81.5 | 98496 |
| | [`LapackSVDSolver`](@ref) | 1100 | 681344 | **4.04** | **0** |
| 384 | [`LapackLU`](@ref)\\* | 601 | 0 | 25.5 | 0 |
| | `LapackPivotedQR` | **7201** | 165696 | 475 | 98496 |
| | [`LapackSVDSolver`](@ref) | 16934 | 5959008 | **35.0** | **0** |
...
that number does not move with the matrix. [`LapackSVDSolver`](@ref)'s solve is two `gemv`s and
allocates nothing.
...
[`LapackSVDSolver`](@ref), as is any question about the spectrum itself.
...
`ComplexF64`); anything else raises an `ArgumentError` naming the type. [`PivotedQR`](@ref) is
the pure-Julia counterpart that takes any floating-point type, the same relationship
[`LU`](@ref) has to [`LapackLU`](@ref). Square matrices only:
...
julia> x = solve(LapackPivotedQR(), A, b);
...
struct LapackPivotedQR{RT} <: RankRevealingMethod{RT}
    ...
    LapackPivotedQR(; rtol = missing) = new{typeof(rtol)}(rtol)
end

"""
    LapackPivotedQRCache <: LinearSolverCache

The cache of a [`LapackPivotedQR`](@ref).
...
[`PivotedLUMethod`](@ref)'s is — see the table in [`LapackPivotedQR`](@ref) for what that costs and
...
32-bit-integer BLAS.
"""
mutable struct LapackPivotedQRCache{T, AT <: AbstractMatrix{T}} <: LinearSolverCache{T}
    A::AT
...
function LinearSolverCache(method::LapackPivotedQR, A::AbstractMatrix{T}) where {T}
    _blas_eltype_check(method, T)
    n = checksquare(A)
    Ā = Matrix{T}(A)
    LapackPivotedQRCache{T, typeof(Ā)}(
        Ā, zeros(LinearAlgebra.BlasInt, n), Vector{T}(undef, n),
        T[], zeros(T, n, 1), 0, false)
end

function _decompose!(method::LapackPivotedQR, c::LapackPivotedQRCache{T}) where {T}
    n = size(c.A, 2)
...
function LinearAlgebra.ldiv!(x::AbstractVector{T}, lsolver::LinearSolver{T, LSM},
        b::AbstractVector{T}) where {T, LSM <: LapackPivotedQR}
    c = cache(lsolver)
