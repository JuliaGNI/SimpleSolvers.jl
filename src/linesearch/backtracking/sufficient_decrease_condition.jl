...
"""
mutable struct SufficientDecreaseCondition{T,VT<:Union{T,AbstractArray{T}},TVT<:Union{T,AbstractArray{T}},PT<:Union{OptimizerProblem{T},LinesearchProblem{T}}} <: BacktrackingCondition{T}
    c::T
    xₖ::VT
    fₖ::T
    gradₖ::TVT
    ...
    function SufficientDecreaseCondition(c₁::T₁, xₖ::VT, fₖ::T, gradₖ::TVT, pₖ::TVT, problem::PT) where {
        T₁<:Number,
        T<:Number,
        VT<:Union{T₁,AbstractArray{T₁}},
        TVT<:Union{T,AbstractArray{T}},
        PT<:Union{OptimizerProblem{T},LinesearchProblem{T}}}
        T != T₁ ? (@warn "You are computing with mixed precision ($(T) and $(T₁)). This is probably not intended.") : nothing
        xₖ_transformed = T.(xₖ)
        new{T,typeof(xₖ_transformed),TVT,PT}(T(c₁), xₖ_transformed, fₖ, gradₖ, pₖ, problem)
    end
end

function (sdc::SufficientDecreaseCondition{T,VT})(xₖ₊₁::VT, αₖ::T) where {T,VT}
    fₖ₊₁ = value(sdc.problem, xₖ₊₁)
