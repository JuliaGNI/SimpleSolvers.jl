...
Abstract type comprising the conditions that are used for checking step sizes for the backtracking line search (see [`Backtracking`](@ref)). This comprises the [`SufficientDecreaseCondition`](@ref) and the [`CurvatureCondition`](@ref).
"""
abstract type BacktrackingCondition{T} end

function (bc::BacktrackingCondition)(xₖ, αₖ, gradₖ)
    error("Condition $(typeof(bc)) not defined for this combination of input arguments.")
end
...
function compute_new_iterate!(xₖ₊₁::VT1,
        xₖ::VT2,
        αₖ::T,
        pₖ::TVT) where {T <: Number, VT1 <: AbstractArray{T},
        VT2 <: AbstractArray{T}, TVT <: AbstractArray{T}}
    @assert axes(xₖ₊₁) == axes(xₖ) == axes(pₖ)
    xₖ₊₁ .= xₖ .+ αₖ .* pₖ
end

function compute_new_iterate!(xₖ::VT,
        αₖ::T,
        pₖ::TVT) where {
        T <: Number, VT <: Union{T, AbstractArray{T}}, TVT <: Union{T, AbstractArray{T}}}
    compute_new_iterate!(xₖ, xₖ, αₖ, pₖ)
end

function compute_new_iterate!(xₖ::VT,
        αₖ::T₁,
        pₖ::TVT) where {T₁ <: Number, T <: Number, VT <: Union{T, AbstractArray{T}},
