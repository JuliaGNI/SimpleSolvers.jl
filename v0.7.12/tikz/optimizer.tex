\documentclass[tikz,border=10pt]{standalone}
\usetikzlibrary{positioning, mindmap}

\definecolor{morange}{RGB}{255,127,14}
\definecolor{mblue}{RGB}{31,119,180}
\definecolor{mred}{RGB}{214,39,40}
\definecolor{mpurple}{RGB}{148,103,189}
\definecolor{mgreen}{RGB}{44,160,44}

\begin{document}
\begin{tikzpicture}[mindmap, concept color=black, text=white,%
                   level 1 concept/.append style={sibling angle=50},%
                   level 2 concept/.append style={sibling angle=50}]
  \path[mindmap,concept color=black,text=white]
    node[concept] {\texttt{Optimizer <: \\AbstractSolver}}
    [clockwise from=0]
    % note that `sibling angle' can only be defined in
    % `level 1 concept/.append style={}'
    child[concept color=mgreen] {node[concept] {\texttt{hessian::\\Hessian}}}
    % note that the `concept color' is passed to the `child'(!)
    child[concept color=mblue] {
      node[concept] {\texttt{problem::\\Optimizer-\\Problem}}
      [clockwise from=-30]
      child { node[concept] {\texttt{g::Gradient}} }
      child { node[concept] {\texttt{h::Hessian}}}
    }
    child[concept color=mred] {node[concept] {\texttt{algorithm::\\Optimizer-\\Method}} }
    child[concept color=mgreen] {node[concept] {\texttt{gradien::\\Gradient}} }
    child[concept color=mpurple] {node[concept] {\texttt{config::\\Options}}}
    child[concept color=morange!] {node[concept] {\texttt{cache::\\Optimizer-\\Cache}}}
    child[concept color=morange!50!mgreen] {node[concept] {\texttt{linesearch::\\Linesearch-\\State}}};
\end{tikzpicture}
\end{document}